...
procedure TSearchForm.Eject(Drive: String);
...
function TSearchForm.AddArtistToCatalogue(Artist : String) : Integer;
begin
//  Artists.Open;
...
function TSearchForm.AddAlbumToCatalogue(Album : String; ArtistID : Integer) : Integer;
begin
//  Albums.Open;
...
function TSearchForm.AddGenreToCatalogue(Genre : String) : Integer;
begin
//  Genres.Open;
...
function TSearchForm.AddCatalogueToCatalogue(Catalogue, CatalogueNumber : String) : Integer;
begin
//  Catalogues.Open;
...
procedure TSearchForm.AddMp3ToCatalogue(Mp3Filename, CatalogueName, CatalogueNumber : String);
...
procedure TSearchForm.ToggleItems(Enabled : Boolean);
begin
...
procedure TSearchForm.bCancelClick(Sender: TObject);
begin
  CancelAdd := true;
  close;
end;

procedure TSearchForm.AddCatalogue;
var i : Integer;
begin
  CancelAdd := False;
  ToggleItems(False);
...
procedure TSearchForm.EasyFileSearch1FileFound(FileFound: TFileInformations);
begin
  if CancelAdd = true then exit;
...
procedure TSearchForm.ceDirectoryChange(Sender: TObject);
begin
...
procedure TSearchForm.ceDirectoryButtonClick(Sender: TObject);
begin
  if PBFolderDialog.Execute then ceDirectory.Text := PBFolderDialog.Folder;
  ceDirectoryChange(nil);
end;

procedure TSearchForm.bNextClick(Sender: TObject);
begin
  ButtonClick(bNext);
end;

procedure TSearchForm.bPreviousClick(Sender: TObject);
begin
  ButtonClick(bPrevious);
end;

procedure TSearchForm.ButtonClick(Sender: TObject);
begin
...
procedure TSearchForm.bCloseClick(Sender: TObject);
begin
  CancelAdd := true;
  close;
end;

procedure TSearchForm.edCatalogueVolNameChange(Sender: TObject);
begin
  bNext.enabled := (length(edCatalogueID.Text) > 0) and (length(edCatalogueVolName.Text) > 0);
end;

procedure TSearchForm.edCatalogueIDChange(Sender: TObject);
begin
  bNext.enabled := (length(edCatalogueID.Text) > 0) and (length(edCatalogueVolName.Text) > 0);
end;

function TSearchForm.EasyFileSearch1AcceptFile(FileFound: TFileInformations): Boolean;
begin
  application.ProcessMessages;
end;

procedure TSearchForm.UpdateProgressList(MP3Filename : String);
begin
  lbProgressItems.Items.Add(MinimizeNameForCaption(Mp3Filename, 294));
  lbProgressItems.ItemIndex := lbProgressItems.Items.Count - 1;
end;

function TSearchForm.CheckGenre(GenreString : String) : String;
...
function TSearchForm.CheckYear(YearString : String) : Integer;
begin
...
function TSearchForm.GetTrackLength(Mp3Filename : String) : String;
var Min,Sec : Integer;
begin
...
function TSearchForm.GetTrackNumber(TrackString : String) : Integer;
...
procedure TSearchForm.edCatalogueIDKeyPress(Sender: TObject;
  var Key: Char);
begin
 if ((ord(Key) > 57) or (ord(Key) < 48)) and not(ord(Key) = 8) then Key := chr(0);
end;

procedure TSearchForm.NewBitBtnClick(Sender: TObject);
...
procedure TSearchForm.GetDriveInfo(Drive : string);
var
  TmpRootPathName,
  TmpVolumeName,
...
procedure TSearchForm.GetDriveShortcuts;
...
procedure TSearchForm.PBFolderDialogInitialized(Sender: TPBFolderDialog;
  DialogHandle: HWND);
begin
  if MainForm.Percents <> 100 then
